###
### Automatically generated via update_3p_localization.py script using files from 
### https://codeberg.org/CrowIsTaken/PlayniteExtensions/tree/b1c22baa59c1c86f2219109d9a9316b626fc8db1 and 
### https://codeberg.org/CrowIsTaken/PlayniteExtensions/tree/b1c22baa59c1c86f2219109d9a9316b626fc8db1. 
### DO NOT MODIFY, CUZ IT MIGHT BE OVERWRITTEN DURING NEXT RUN!
###
third-party-playnite-game-start-error = Non foi posíbel comezar o xogo: { $var0 }
third-party-playnite-game-install-error = Non foi posíbel instalar o xogo: { $var0 }
third-party-playnite-game-uninstall-error = Non foi posíbel desinstalar o xogo: { $var0 }
third-party-playnite-version-label = Versión
third-party-playnite-ok-label = OK
third-party-playnite-save-label = Gardar
third-party-playnite-cancel-label = Cancelar
third-party-playnite-yes-label = Sí
third-party-playnite-no-label = Non
third-party-playnite-downloading-label = Descarregando...
third-party-playnite-loading-label = Cargando...
third-party-playnite-settings-label = Axustes
third-party-playnite-settings-general-label = Xeral
third-party-playnite-settings-advanced-label = Avanzado
third-party-playnite-settings-clear-cache-title = Borrar a caché?
third-party-playnite-settings-playtime-import-mode-never = Nunca
third-party-playnite-login-required = Requírese autenticación
third-party-playnite-executable-title = Executábel
third-party-playnite-game-install-dir-title = Cartafol de Instalación
third-party-playnite-library-import-error = Erro ó importar os xogos de { $var0 }.
third-party-playnite-metadata-download-error = Fallou ó descargar os metadatos: { $var0 }
third-party-playnite-uninstalling = Desinstalando
third-party-playnite-game-launching = Lanzando
third-party-playnite-do-nothing = Non facer nada
third-party-playnite-updater-window-title = Actualización dispoñible
third-party-playnite-updater-install-update = Descargar e instalar a actualización
third-party-playnite-check-for-updates = Buscar actualizacións
third-party-playnite-update-check-fail-message = Error ó comprobar actualizacións.
third-party-playnite-install-game = Instalar
third-party-playnite-uninstall-game = Desinstalar
third-party-playnite-dont-show-again-title = Non mostrar de novo
third-party-playnite-extensions-updates = Actualizacións
third-party-playnite-select-file-tooltip = Selecciona o ficheiro…
third-party-playnite-select-directory-tooltip = Selecciona o cartafol…
third-party-playnite-option-only-manually = Só manualmente
third-party-playnite-option-once-a-day = Diariamente
third-party-playnite-option-once-a-week = Semanalmente
third-party-playnite-open = Abrir
third-party-amazon-settings-import-installed-label = Importar xogos instalados
third-party-amazon-settings-import-uninstalled-label = Importar xogos non instalados
third-party-amazon-settings-connect-account = Vincular conta
third-party-amazon-login-checking = Comprobando estado da verificación
third-party-amazon-logged-in = O usuario xa está verificado
third-party-amazon-not-logged-in = Requírese verificación
third-party-amazon-not-logged-in-error = Erro na verificación do usuario
third-party-amazon-authenticate-label = Verificar
third-party-amazon-settings-start-game-directly = Comezar xogos dereitamente sen arrincar o cliente oficial
third-party-amazon-trouble-shooting-account-link = Solución de problemas coa sincronización da conta
third-party-amazon-trouble-shooting-issues = Solución de problemas
third-party-amazon-settings-import-library-label = Importar tódolos xogos
third-party-amazon-private-account = Conta privada
third-party-amazon-public-account = Conta pública
third-party-amazon-api-key = Clave da API
third-party-amazon-start-using-client = Comezar usando o cliente { $var0 }
