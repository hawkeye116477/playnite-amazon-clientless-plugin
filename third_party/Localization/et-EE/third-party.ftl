###
### Automatically generated via update_3p_localization.py script using files from 
### https://codeberg.org/CrowIsTaken/PlayniteExtensions/tree/b1c22baa59c1c86f2219109d9a9316b626fc8db1 and 
### https://codeberg.org/CrowIsTaken/PlayniteExtensions/tree/b1c22baa59c1c86f2219109d9a9316b626fc8db1. 
### DO NOT MODIFY, CUZ IT MIGHT BE OVERWRITTEN DURING NEXT RUN!
###
third-party-playnite-game-start-error = Ei saa käivitada mängu: { $var0 }
third-party-playnite-game-install-error = Ei saa installida mängu: { $var0 }
third-party-playnite-game-uninstall-error = Ei saa desinstallida mängu: { $var0 }
third-party-playnite-version-label = Versioon
third-party-playnite-ok-label = OK
third-party-playnite-save-label = Salvesta
third-party-playnite-cancel-label = Tühista
third-party-playnite-yes-label = Jah
third-party-playnite-no-label = Ei
third-party-playnite-downloading-label = Allalaadimine…
third-party-playnite-loading-label = Laadimine…
third-party-playnite-settings-label = Seaded
third-party-playnite-settings-general-label = Üldine
third-party-playnite-settings-advanced-label = Täpsem
third-party-playnite-settings-clear-cache-title = Tühjenda vahemälu?
third-party-playnite-settings-playtime-import-mode-never = Mitte kunagi
third-party-playnite-login-required = Autentimine vajalik
third-party-playnite-executable-title = Täitmisprogramm
third-party-playnite-game-install-dir-title = Paigalduse asukoht
third-party-playnite-library-import-error = Ei saanud importida mänge kohast { $var0 }.
third-party-playnite-metadata-download-error = Metaandmete allalaadimine ebaõnnestus: { $var0 }
third-party-playnite-uninstalling = Desinstallimine
third-party-playnite-game-launching = Käivitamine
third-party-playnite-do-nothing = Ära tee midagi
third-party-playnite-updater-window-title = Värskendus on saadaval
third-party-playnite-updater-install-update = Laadi alla ja paigalda värskendus
third-party-playnite-check-for-updates = Otsi värskendusi
third-party-playnite-update-check-fail-message = Uue versiooni olemasolu kontroll ebaõnnestus.
third-party-playnite-install-game = Paigalda
third-party-playnite-uninstall-game = Desinstalli
third-party-playnite-dont-show-again-title = Ära enam näita
third-party-playnite-extensions-updates = Värskendused
third-party-playnite-select-file-tooltip = Vali fail…
third-party-playnite-select-directory-tooltip = Vali kaust…
third-party-playnite-option-only-manually = Ainult käsitsi
third-party-playnite-option-once-a-day = Kord päevas
third-party-playnite-option-once-a-week = Kord nädalas
third-party-playnite-open = Ava
third-party-amazon-settings-import-installed-label = Impordi paigaldatud mängud
third-party-amazon-settings-import-uninstalled-label = Impordi paigaldamata mängud
third-party-amazon-settings-connect-account = Ühenda konto
third-party-amazon-login-checking = Autentimise oleku kontrollimine…
third-party-amazon-logged-in = Kasutaja on autenditud
third-party-amazon-not-logged-in = Vajalik on autentimine
third-party-amazon-not-logged-in-error = Kasutaja autentimine ebaõnnestus.
third-party-amazon-authenticate-label = Autendi
third-party-amazon-settings-start-game-directly = Käivita mängud otseselt, ilma ametlikku klienti avamata
third-party-amazon-trouble-shooting-account-link = Konto sünkroonimise probleemide tõrkeotsing
third-party-amazon-trouble-shooting-issues = Probleemide tõrkeotsing
third-party-amazon-settings-import-library-label = Impordi kõik mängud
third-party-amazon-private-account = Privaatne konto
third-party-amazon-public-account = Avalik konto
third-party-amazon-api-key = API võti
third-party-amazon-start-using-client = Käivita klienti { $var0 } kasutades
