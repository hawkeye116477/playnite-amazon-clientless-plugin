###
### Automatically generated via update_3p_localization.py script using files from 
### https://codeberg.org/CrowIsTaken/PlayniteExtensions/tree/b1c22baa59c1c86f2219109d9a9316b626fc8db1 and 
### https://codeberg.org/CrowIsTaken/PlayniteExtensions/tree/b1c22baa59c1c86f2219109d9a9316b626fc8db1. 
### DO NOT MODIFY, CUZ IT MIGHT BE OVERWRITTEN DURING NEXT RUN!
###
third-party-playnite-game-start-error = Kan ikke starte spillet: { $var0 }
third-party-playnite-game-install-error = Kan ikke installere spil: { $var0 }
third-party-playnite-game-uninstall-error = Kan ikke afinstallere spil: { $var0 }
third-party-playnite-version-label = Version
third-party-playnite-ok-label = OK
third-party-playnite-save-label = Gem
third-party-playnite-cancel-label = Annullér
third-party-playnite-yes-label = Ja
third-party-playnite-no-label = Nej
third-party-playnite-downloading-label = Downloader…
third-party-playnite-loading-label = Indlæser…
third-party-playnite-settings-label = Indstillinger
third-party-playnite-settings-general-label = Generelt
third-party-playnite-settings-advanced-label = Avanceret
third-party-playnite-settings-clear-cache-title = Ryd cache?
third-party-playnite-login-required = Godkendelse krævet
third-party-playnite-executable-title = Eksekverings-fil
third-party-playnite-uninstalling = Afinstallerer
third-party-playnite-game-launching = Starter
third-party-playnite-do-nothing = Gør ingenting
third-party-playnite-install-game = Installér
third-party-playnite-uninstall-game = Afinstaller
third-party-amazon-settings-import-installed-label = Importer installerede spil
third-party-amazon-settings-import-uninstalled-label = Importer ikke installerede spil
third-party-amazon-settings-connect-account = Forbind konto
third-party-amazon-login-checking = Tjekker godkendelsesstatus…
third-party-amazon-logged-in = Brugeren er verificeret
third-party-amazon-not-logged-in = Kræver verificering
third-party-amazon-not-logged-in-error = Det lykkedes ikke at verificere brugeren
third-party-amazon-authenticate-label = Verificering
third-party-amazon-settings-start-game-directly = Start spil direkte uden at køre officiel klient
third-party-amazon-trouble-shooting-account-link = Fejlfinding for konto synkroniserings problemer
third-party-amazon-trouble-shooting-issues = Problemer med fejlfinding
third-party-amazon-settings-import-library-label = Importer alle spil
third-party-amazon-private-account = Privat konto
third-party-amazon-public-account = Offentlig Konto
third-party-amazon-api-key = API Nøgle
third-party-amazon-start-using-client = Begynd at bruge { $var0 } klient
