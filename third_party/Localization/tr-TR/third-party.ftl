###
### Automatically generated via update_3p_localization.py script using files from 
### https://codeberg.org/CrowIsTaken/PlayniteExtensions/tree/b1c22baa59c1c86f2219109d9a9316b626fc8db1 and 
### https://codeberg.org/CrowIsTaken/PlayniteExtensions/tree/b1c22baa59c1c86f2219109d9a9316b626fc8db1. 
### DO NOT MODIFY, CUZ IT MIGHT BE OVERWRITTEN DURING NEXT RUN!
###
third-party-playnite-game-start-error = Oyun başlatılamıyor: { $var0 }
third-party-playnite-game-install-error = Oyun kurulamıyor: { $var0 }
third-party-playnite-game-uninstall-error = Oyun kaldırılamıyor: { $var0 }
third-party-playnite-version-label = Sürüm
third-party-playnite-ok-label = TAMAM
third-party-playnite-save-label = Kaydet
third-party-playnite-cancel-label = İptal
third-party-playnite-yes-label = Evet
third-party-playnite-no-label = Hayır
third-party-playnite-downloading-label = İndiriliyor…
third-party-playnite-loading-label = Yükleniyor…
third-party-playnite-settings-label = Ayarlar
third-party-playnite-settings-general-label = Genel
third-party-playnite-settings-advanced-label = Gelişmiş
third-party-playnite-settings-clear-cache-title = Önbelleği Temizle?
third-party-playnite-settings-playtime-import-mode-never = Asla
third-party-playnite-login-required = Doğrulama gerekli
third-party-playnite-executable-title = Uygulama
third-party-playnite-game-install-dir-title = Kurulum Klasörü
third-party-playnite-library-import-error = { $var0 } konumundan oyun içe aktarılamadı.
third-party-playnite-metadata-download-error = Üst veriler indirilemedi: { $var0 }
third-party-playnite-uninstalling = Kaldırılıyor
third-party-playnite-game-launching = Başlatılıyor
third-party-playnite-do-nothing = Hiçbir şey yapma
third-party-playnite-updater-window-title = Güncelleme Mevcut
third-party-playnite-updater-install-update = Güncellemeyi İndir ve Kur
third-party-playnite-check-for-updates = Güncelleştirmeleri Denetle
third-party-playnite-update-check-fail-message = Yeni sürüm kontrol edilemedi.
third-party-playnite-install-game = Kur
third-party-playnite-uninstall-game = Kurulumu Kaldır
third-party-playnite-dont-show-again-title = Bir Daha Gösterme
third-party-playnite-extensions-updates = Güncellemeler
third-party-playnite-select-file-tooltip = Dosya seç…
third-party-playnite-select-directory-tooltip = Klasör seç…
third-party-playnite-option-only-manually = Sadece el ile
third-party-playnite-option-once-a-day = Günde bir
third-party-playnite-option-once-a-week = Haftada bir
third-party-playnite-open = Aç
third-party-amazon-settings-import-installed-label = Kurulu oyunları içeri aktar
third-party-amazon-settings-import-uninstalled-label = Kurulu olmayan oyunları içe aktar
third-party-amazon-settings-connect-account = Hesap bağla
third-party-amazon-login-checking = Kimlik doğrulama durumu kontrol ediliyor…
third-party-amazon-logged-in = Kullanıcı kimliği doğrulandı
third-party-amazon-not-logged-in = Kimlik doğrulaması gerektiriyor
third-party-amazon-not-logged-in-error = Kullanıcı kimliği doğrulanamadı.
third-party-amazon-authenticate-label = Kimlik Doğrulaması
third-party-amazon-settings-start-game-directly = Oyunu resmi istemciyi çalıştırmadan doğrudan başlat
third-party-amazon-trouble-shooting-account-link = Hesap eşitleme sorunlarını giderme
third-party-amazon-trouble-shooting-issues = Sorunları giderme
third-party-amazon-settings-import-library-label = Tüm oyunları içeri aktar
third-party-amazon-private-account = Özel Hesap
third-party-amazon-public-account = Genel Hesap
third-party-amazon-api-key = API Anahtarı
third-party-amazon-start-using-client = { $var0 } istemcisini kullanarak başlat
