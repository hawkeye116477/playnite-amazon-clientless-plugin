###
### Automatically generated via update_3p_localization.py script using files from 
### https://codeberg.org/CrowIsTaken/PlayniteExtensions/tree/b1c22baa59c1c86f2219109d9a9316b626fc8db1 and 
### https://codeberg.org/CrowIsTaken/PlayniteExtensions/tree/b1c22baa59c1c86f2219109d9a9316b626fc8db1. 
### DO NOT MODIFY, CUZ IT MIGHT BE OVERWRITTEN DURING NEXT RUN!
###
third-party-playnite-game-start-error = Nu poate fi pornită camera: { $var0 }
third-party-playnite-game-install-error = Nu se poate instala jocul: { $var0 }
third-party-playnite-game-uninstall-error = Nu se poate dezinstala jocul: { $var0 }
third-party-playnite-version-label = Versiune
third-party-playnite-ok-label = OK
third-party-playnite-save-label = Salvează
third-party-playnite-cancel-label = Anulează
third-party-playnite-yes-label = Da
third-party-playnite-no-label = Nu
third-party-playnite-downloading-label = Se descarcă...
third-party-playnite-loading-label = Se încarcă...
third-party-playnite-settings-label = Setări
third-party-playnite-settings-general-label = General
third-party-playnite-settings-advanced-label = Avansat
third-party-playnite-settings-clear-cache-title = Curățați Cache-ul?
third-party-playnite-settings-playtime-import-mode-never = Niciodată
third-party-playnite-login-required = Autentificare necesară
third-party-playnite-executable-title = Executabil
third-party-playnite-game-install-dir-title = Directorul de instalare
third-party-playnite-library-import-error = Nu s-a putut importa jocuri de la { $var0 }.
third-party-playnite-metadata-download-error = Nu s-a putut descărca metadata: { $var0 }
third-party-playnite-uninstalling = Se dezinstalează
third-party-playnite-game-launching = Se lansează
third-party-playnite-do-nothing = Repaus
third-party-playnite-updater-window-title = Versiune nouă disponibilă
third-party-playnite-updater-install-update = Descarcă şi actualizează programul
third-party-playnite-check-for-updates = Verificați dacă sunt disponibile Actualizări
third-party-playnite-update-check-fail-message = Nu s-a putut căuta versiunea nouă.
third-party-playnite-install-game = Instalaţi
third-party-playnite-uninstall-game = Dezinstalați
third-party-playnite-dont-show-again-title = Nu mai afișa
third-party-amazon-settings-import-installed-label = Importați jocurile instalate
third-party-amazon-settings-import-uninstalled-label = Importanți jocurile neinstalate
third-party-amazon-settings-connect-account = Conectare cont
third-party-amazon-login-checking = Se verifică starea de autentificare...
third-party-amazon-logged-in = Utilizatorul este autentificat
third-party-amazon-not-logged-in = Necesită autentificare
third-party-amazon-not-logged-in-error = Autentificare eșuată.
third-party-amazon-authenticate-label = Autentificați-vă
third-party-amazon-settings-start-game-directly = Începe jocurile direct fără a rula clientul oficial
third-party-amazon-trouble-shooting-account-link = Depanarea problemelor de sincronizare a contului
third-party-amazon-trouble-shooting-issues = Probleme de depanare
third-party-amazon-settings-import-library-label = Importați toate jocurile
third-party-amazon-private-account = Cont privat
third-party-amazon-public-account = Cont public
third-party-amazon-api-key = Cheie API
third-party-amazon-start-using-client = Începeți să folosiți clientul { $var0 }
