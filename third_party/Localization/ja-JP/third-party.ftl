###
### Automatically generated via update_3p_localization.py script using files from 
### https://codeberg.org/CrowIsTaken/PlayniteExtensions/tree/b1c22baa59c1c86f2219109d9a9316b626fc8db1 and 
### https://codeberg.org/CrowIsTaken/PlayniteExtensions/tree/b1c22baa59c1c86f2219109d9a9316b626fc8db1. 
### DO NOT MODIFY, CUZ IT MIGHT BE OVERWRITTEN DURING NEXT RUN!
###
third-party-playnite-game-start-error = ゲームを開始できませんでした: { $var0 }
third-party-playnite-game-install-error = ゲームをインストールできません: { $var0 }
third-party-playnite-game-uninstall-error = ゲームをアンインストールできません: { $var0 }
third-party-playnite-version-label = バージョン
third-party-playnite-ok-label = OK
third-party-playnite-save-label = 保存
third-party-playnite-cancel-label = キャンセル
third-party-playnite-yes-label = はい
third-party-playnite-no-label = いいえ
third-party-playnite-downloading-label = ダウンロード中
third-party-playnite-loading-label = 読み込み中…
third-party-playnite-settings-label = 設定
third-party-playnite-settings-general-label = 一般設定
third-party-playnite-settings-advanced-label = 詳細設定
third-party-playnite-settings-clear-cache-title = キャッシュをクリアしますか？
third-party-playnite-settings-playtime-import-mode-never = 常に無効
third-party-playnite-login-required = 認証が必要です
third-party-playnite-executable-title = 実行ファイル
third-party-playnite-game-install-dir-title = インストールフォルダ
third-party-playnite-library-import-error = { $var0 } からゲームをインポートできませんでした。
third-party-playnite-metadata-download-error = メタデータをダウンロードできませんでした: { $var0 }
third-party-playnite-uninstalling = アンインストール中
third-party-playnite-game-launching = 起動中
third-party-playnite-do-nothing = 何もしない
third-party-playnite-updater-window-title = 更新が利用可能です
third-party-playnite-updater-install-update = 更新をダウンロードしてインストール
third-party-playnite-check-for-updates = 更新を確認
third-party-playnite-update-check-fail-message = 新しいバージョンを確認できませんでした。
third-party-playnite-install-game = インストール
third-party-playnite-uninstall-game = アンインストール
third-party-playnite-dont-show-again-title = 次回から表示しない
third-party-playnite-extensions-updates = 更新
third-party-playnite-select-file-tooltip = ファイルを選択
third-party-playnite-select-directory-tooltip = フォルダを選択…
third-party-playnite-option-only-manually = 手動のみ
third-party-playnite-option-once-a-day = 1日に1回
third-party-playnite-option-once-a-week = 週に1回
third-party-playnite-open = 開く
third-party-amazon-settings-import-installed-label = インストール済みのゲームをインポート
third-party-amazon-settings-import-uninstalled-label = インストールされていないゲームをインポート
third-party-amazon-settings-connect-account = アカウントに接続
third-party-amazon-login-checking = 認証ステータスを確認しています...
third-party-amazon-logged-in = ユーザーは認証されています
third-party-amazon-not-logged-in = 認証が必要です
third-party-amazon-not-logged-in-error = ユーザー認証に失敗しました。
third-party-amazon-authenticate-label = 認証
third-party-amazon-settings-start-game-directly = 公式クライアントを起動しないでゲームを直接起動する
third-party-amazon-trouble-shooting-account-link = アカウント同期問題のトラブルシューティング
third-party-amazon-trouble-shooting-issues = 問題のトラブルシューティング
third-party-amazon-settings-import-library-label = すべてのゲームをインポート
third-party-amazon-private-account = 非公開アカウント
third-party-amazon-public-account = 公開アカウント
third-party-amazon-api-key = API キー
third-party-amazon-start-using-client = { $var0 } クライアントの使用を開始
