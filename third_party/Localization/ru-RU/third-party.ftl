###
### Automatically generated via update_3p_localization.py script using files from 
### https://codeberg.org/CrowIsTaken/PlayniteExtensions/tree/b1c22baa59c1c86f2219109d9a9316b626fc8db1 and 
### https://codeberg.org/CrowIsTaken/PlayniteExtensions/tree/b1c22baa59c1c86f2219109d9a9316b626fc8db1. 
### DO NOT MODIFY, CUZ IT MIGHT BE OVERWRITTEN DURING NEXT RUN!
###
third-party-playnite-game-start-error = Не удалось запустить игру: { $var0 }
third-party-playnite-game-install-error = Не удалось установить игру: { $var0 }
third-party-playnite-game-uninstall-error = Не удалось удалить игру: { $var0 }
third-party-playnite-version-label = Версия
third-party-playnite-ok-label = Окей
third-party-playnite-save-label = Сохранить
third-party-playnite-cancel-label = Отменить
third-party-playnite-yes-label = Да
third-party-playnite-no-label = Нет
third-party-playnite-downloading-label = Скачивание…
third-party-playnite-loading-label = Загрузка…
third-party-playnite-settings-label = Настройки
third-party-playnite-settings-general-label = Основные
third-party-playnite-settings-advanced-label = Расширенные
third-party-playnite-settings-clear-cache-title = Очистить кэш?
third-party-playnite-settings-playtime-import-mode-never = Никогда
third-party-playnite-login-required = Требуется авторизация
third-party-playnite-executable-title = Исполняемый файл
third-party-playnite-game-install-dir-title = Путь установки
third-party-playnite-library-import-error = Не удалось импортировать игры из { $var0 }.
third-party-playnite-metadata-download-error = Не удалось скачать метаданные: { $var0 }
third-party-playnite-uninstalling = Удаление
third-party-playnite-game-launching = Запускается
third-party-playnite-do-nothing = Ничего не делать
third-party-playnite-updater-window-title = Вышла новая версия
third-party-playnite-updater-install-update = Скачать и обновить
third-party-playnite-check-for-updates = Проверить наличие обновлений
third-party-playnite-update-check-fail-message = Не удалось проверить наличие новой версии.
third-party-playnite-install-game = Установить
third-party-playnite-uninstall-game = Удалить с устройства
third-party-playnite-dont-show-again-title = Больше не показывать
third-party-playnite-extensions-updates = Обновления
third-party-playnite-select-file-tooltip = Выбрать файл…
third-party-playnite-select-directory-tooltip = Выбрать папку…
third-party-playnite-option-only-manually = Только вручную
third-party-playnite-option-once-a-day = Раз в день
third-party-playnite-option-once-a-week = Раз в неделю
third-party-playnite-open = Открыть
third-party-amazon-settings-import-installed-label = Импортировать установленные игры
third-party-amazon-settings-import-uninstalled-label = Импортировать неустановленные игры
third-party-amazon-settings-connect-account = Подключить аккаунт
third-party-amazon-login-checking = Проверка статуса аутентификации…
third-party-amazon-logged-in = Пользователь авторизован
third-party-amazon-not-logged-in = Требуется авторизация
third-party-amazon-not-logged-in-error = Не удалось авторизовать пользователя.
third-party-amazon-authenticate-label = Аутентификация
third-party-amazon-settings-start-game-directly = Запускать игры напрямую через .exe, в обход официального клиента
third-party-amazon-trouble-shooting-account-link = Устранение неполадок синхронизации учетной записи
third-party-amazon-trouble-shooting-issues = Устранение неполадок
third-party-amazon-settings-import-library-label = Импортировать все игры
third-party-amazon-private-account = Закрытый профиль
third-party-amazon-public-account = Открытый профиль
third-party-amazon-api-key = API-ключ
third-party-amazon-start-using-client = Запустить через клиент { $var0 }
