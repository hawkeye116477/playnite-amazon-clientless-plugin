...
amazon_loc_keys = {}
with open(pj(script_path, "config", "amazonLocKeys.txt"),
          "r", encoding="utf-8") as amazon_loc_keys_content:
    for line in amazon_loc_keys_content:
        if line := line.strip():
            amazon_loc_keys[line] = ""
...
for filename in os.listdir(pj(main_path, "..", "..", "PlayniteExtensions", "PlayniteRepo", "source", "Playnite", "Localization")):
    ...
    if filename not in ["LocSource.xaml", "LocalizationKeys.cs", "locstatus.json"]:
        ...
        for child in playnite_loc.getroot():
            ...
            if key in playnite_loc_keys:
                ...
                new_key.set(ET.QName(xmlns_x, "Key"), key.replace("LOC", "LOCNile3P_Playnite"))
    ...
    if filename not in ["LocSource.xaml", "LocalizationKeys.cs", "locstatus.json"]:
        ...
        amazon_file_path = pj(main_path, "..",  "..", "PlayniteExtensions",
                            "source", "Libraries", "AmazonGamesLibrary", "Localization", filename)
        if os.path.isfile(amazon_file_path):
            amazon_loc = ET.parse(amazon_file_path)
            for child in amazon_loc.getroot():
                ...
                if key in amazon_loc_keys:
                    ...
                    if not key_text:
                        key_text = ""
                    new_key = ET.Element(ET.QName(xmlns_sys, "String"))
                    new_key.set(ET.QName(xmlns_x, "Key"), key.replace("LOCAmazon", "LOCNile3P_Amazon"))
                    new_key.text = key_text
                    if key_text != "":
                        xml_root.append(new_key)
...
"""
        if len(xml_doc.getroot()) > 0:
            with open(pj(localization_path, loc_sub_dir, "third-party.ftl"), "w", encoding="utf-8") as i18n_file:
                ftl_new_content = convertToFtl_script.convertXamlStringToFtl(xml_doc, "Nile")
...
copy_specific_named_files_with_subdirs(common_loc_path_source, pj(third_party_path, "CommonLocalization"), file_names=["common.ftl", "common-libplugin.ftl"])
